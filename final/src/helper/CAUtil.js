...
exports.registerAndEnrollUserMongo = async(caClient, wallet, orgMspId, userId, affiliation) => {
    try {
        console.log("---register user start--", userId.toString())
        //     Check to see if we've already enrolled the user
        const userIdentity = await wallet.get(userId);
        if (userIdentity) {
            console.log(`An identity for the user ${userId} already exists in the wallet`);
...
exports.enrollAdminMongo = async(caClient, wallet, orgMspId, adminUserId) => {
    try {
        // Check to see if we've already enrolled the admin user.
        // let adminUserId = `adminUserId${orgMspId}`
        const identity = await wallet.get(adminUserId);
        if (identity) {
            console.log('An identity for the admin user already exists in the wallet');
            return;
        ...
        await wallet.put(adminUserId, data);
        console.log('Successfully enrolled admin user and imported it into the wallet');
    } catch (error) {
        console.error(`Failed to enroll admin user : ${error}`);
    }
