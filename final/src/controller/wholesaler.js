...
exports.addproductwholesaler = async(req, res) => {
    data = parseJwt(req.cookies.jwt);
    console.log(data);
    const {productId, wholesalerId, shiptoretailerdate, retailerId} = req.body;
    console.log('---------x', productId, wholesalerId, shiptoretailerdate, retailerId)
    const db = await User.find({email: data.email});
    const orgmsp = db[0].org
    // const orgmsp = "Org1MSP";
    let userid = db[0]._id.toString();
    ...
    try{
        ...
        console.log('\n--> Submit Transaction: Add Product changes using wholesaler');
        let result = await contract.submitTransaction('addProduct_wholesaler', productId, wholesalerId, shiptoretailerdate, retailerId);
        console.log('*** Result: committed');
        if (`${result}` !== '') {
            console.log(`*** Result: ${prettyJSONString(result.toString())}`);
        }
