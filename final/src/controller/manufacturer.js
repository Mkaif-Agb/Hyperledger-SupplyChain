...
const Wallet_Model = require('../models/model_wallet');
const bcrypt = require('bcryptjs')
const Verify = require('../helper/authentication');
const {registerAdmin, registerUser, userExist} = require('../helper/registerUser');
const {buildCCPOrg1, buildCCPOrg2, buildCCPOrg3, buildCCPOrg4, buildCCPOrg5, buildWallet} = require('../helper/AppUtil');
...
const fs = require('fs');
const { registerAndEnrollUserMongo } = require('../helper/CAUtil');
...
exports.addproduct = async(req, res) => {
    data = parseJwt(req.cookies.jwt);
    console.log(data);
    const {productId, productName, mfgDate, extDate, currentOwner, batchId, shiptowholesalerdate} = req.body;
    // console.log('---------x', userId, email, phone, name, type)
    const db = await User.find({email: data.email});
    const orgmsp = db[0].org
    // const orgmsp = "Org1MSP";
    ...
    try{
        ...
        console.log('\n--> Submit Transaction: Add Product');
        let result = await contract.submitTransaction('addProduct', productId, productName, mfgDate, extDate, currentOwner, batchId, shiptowholesalerdate);
        console.log('*** Result: committed');
        if (`${result}` !== '') {
            console.log(`*** Result: ${prettyJSONString(result.toString())}`);
        }
...
exports.addfactory = async(req, res) => {
    ...
    try{
        ...
        console.log('\n--> Submit Transaction: Add Factory, creates new asset with Id, Role and Name');
