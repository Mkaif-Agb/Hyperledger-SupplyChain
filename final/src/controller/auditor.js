...
exports.addProduct_auditor = async(req, res) => {
    data = parseJwt(req.cookies.jwt);
    console.log(data);
    const {productId, qualityAudit} = req.body;
    // console.log('---------x', userId, email, phone, name, type)
    const db = await User.find({email: data.email});
    const orgmsp = db[0].org
    // const orgmsp = "Org1MSP";
    ...
    try{
        ...
        console.log('\n--> Submit Transaction: Make changes to the product by Auditor');
        let result = await contract.submitTransaction('addProduct_auditor', productId, qualityAudit);
        console.log('*** Result: committed');
        if (`${result}` !== '') {
            console.log(`*** Result: ${prettyJSONString(result.toString())}`);
        }
