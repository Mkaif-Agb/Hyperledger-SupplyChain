...
class testContract extends Contract {
    ...
    async addProduct(ctx, productId, productName, mfgDate, extDate, currentOwner, batchId, shiptowholesalerdate){
    ...
    async addProduct_auditor(ctx,productId, qualityAudit){
        ...
        const assetString = await this.ReadAsset(ctx, productId);
        const asset = JSON.parse(assetString);
        asset.QualityAudit =  qualityAudit;
        await ctx.stub.putState(productId, Buffer.from(JSON.stringify(asset)));
    ...
    async addProduct_manufacturer(ctx, productId,shiptowholesalerdate){
        ...
        const assetString = await this.ReadAsset(ctx, productId);
        const asset = JSON.parse(assetString);
        asset.Shiptowholesalerdate = shiptowholesalerdate;
        await ctx.stub.putState(productId, Buffer.from(JSON.stringify(asset)));
    ...
    async addProduct_wholesaler(ctx, productId,wholesalerId, shiptoretailerdate, retailerId){
        ...
        const assetString = await this.ReadAsset(ctx, productId);
        const asset = JSON.parse(assetString);
        asset.WholesalerId = wholesalerId;
        asset.Shiptoretailerdate = shiptoretailerdate;
        asset.RetailerID = retailerId;
        await ctx.stub.putState(productId, Buffer.from(JSON.stringify(asset)));
